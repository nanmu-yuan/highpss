        ...
        <div class="tab_cont1 tab_cont" v-for="(item, index) in info.cont_zj" :key="index" v-show="vShow == 0">
          <div v-show="info.tabShow">
            <div>
              ...
              <div class="costext" v-for="(items, i) in item.courseDetail" :key="i" @mouseenter="showClass(index, i)"
                @mouseleave="hideClass(index, i)" :class="n == i && isActive == index ? 'act' : ''"
                @click="costextPush(index, i, items.id, type)">
                <div>
                  <img src="@/assets/image/langsi/fill.png" alt="" />
                  <p>{{ items.title }}</p>
                </div>
                <div class="btn" v-show="isShow">继续学习</div>
                <div class="btn btn2" v-show="n == i && isActive == index ? trShow : ''">开始学习</div>
              </div>
            </div>
          </div>
          <div v-show="!info.tabShow">
            <div class="costext2" v-for="(item, index) in item.cont_zj2" :key="index">
              <h2>{{ item.title }}</h2>
              <p>{{ item.text1 }}</p>
              <p>{{ item.text2 }}</p>
              <p>{{ item.text3 }}</p>
