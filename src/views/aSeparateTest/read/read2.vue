    ...
    <div class="test">
          ...
          <p style="padding: 0.125rem 0">
            <span v-for="(items, indexs) in item" :key="indexs">
              ...
              <span class="answer1" v-if="items.value || answer[items.id]">
                <i style="font-style: normal; font-size: 0.3rem">{{ items.ids || (answer[items.id] ? answer[items.id].id : '') }}:</i>
                <i style="font-style: normal">{{ items.value || (answer[items.id] ? answer[items.id].value : '') }}</i>
                <i class="iconfont icon-cuowu" style="font-size: 0.275rem; text-align: center; cursor: pointer; margin-left: 0.375rem; font-style: normal" @click="remove(index, indexs, items.id)"></i>
              </span>
              <span class="answer" @drop="drop(index, indexs, items.id)" @dragover="dragover" v-else-if="!items.value && items.state" :style="{ backgroundColor: errors[items.id] == items.id ? '#ff6969' : '' }">{{ isAnalysis ? info.answer[items.id] : '' }}</span>
              <!-- 查看解析 -->
              <Analysis v-show="items.state" @isView="isView(index, indexs)" :flag="items.flag" :analysis="analysis[items.id]" />
            </span>
          </p>
        </div>
      </div>
      ...
      <div class="option fl scrollBar">
        <div v-for="item in subjectList" :key="item.id">
          <div :draggable="draggable[0].flag" :class="{ is_mask: !draggable[0].flag }" @dragstart="dragstart(item.sentenceA, 'A', 0)" v-if="item.sentenceA">
            <div>A</div>
            <p>{{ item.sentenceA }}</p>
          </div>
          <div :draggable="draggable[1].flag" :class="{ is_mask: !draggable[1].flag }" @dragstart="dragstart(item.sentenceB, 'B', 1)" v-if="item.sentenceB">
            <div>B</div>
            <p>{{ item.sentenceB }}</p>
          </div>
          <div :draggable="draggable[2].flag" :class="{ is_mask: !draggable[2].flag }" @dragstart="dragstart(item.sentenceC, 'C', 2)" v-if="item.sentenceC">
            <div>C</div>
            <p>{{ item.sentenceC }}</p>
          </div>
          <div :draggable="draggable[3].flag" :class="{ is_mask: !draggable[3].flag }" @dragstart="dragstart(item.sentenceD, 'D', 3)" v-if="item.sentenceD">
            <div>D</div>
            <p>{{ item.sentenceD }}</p>
          </div>
          <div :draggable="draggable[4].flag" :class="{ is_mask: !draggable[4].flag }" @dragstart="dragstart(item.sentenceE, 'E', 4)" v-if="item.sentenceE">
            <div>E</div>
            <p>{{ item.sentenceE }}</p>
          </div>
          <div :draggable="draggable[5].flag" :class="{ is_mask: !draggable[5].flag }" @dragstart="dragstart(item.sentenceF, 'F', 5)" v-if="item.sentenceF">
            <div>F</div>
            <p>{{ item.sentenceF }}</p>
          </div>
          <div :draggable="draggable[6].flag" :class="{ is_mask: !draggable[6].flag }" @dragstart="dragstart(item.sentenceG, 'G', 6)" v-if="item.sentenceG">
            <div>G</div>
            <p>{{ item.sentenceG }}</p>
          </div>
          <div :draggable="draggable[7].flag" :class="{ is_mask: !draggable[7].flag }" @dragstart="dragstart(item.sentenceH, 'H', 7)" v-if="item.sentenceH">
            <div>H</div>
            <p>{{ item.sentenceH }}</p>
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    // 获取解析
    getAnswer1() {
      this.$api.getreadsubmitInfo(2).then((res) => {
    ...
    async getRead() {
      this.$api.getSubject(2).then((res) => {
        if (res.data.code == 200) {
          ...
          if (this.isAnalysis) {
            this.correct = this.subjectList[0].answer
            this.analysis = this.subjectList[0].analysis
            this.getAnswer1()
          }
        }
      })
