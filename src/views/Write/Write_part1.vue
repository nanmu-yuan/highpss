...
export default {
  ...
  created() {
    ...
    this.info.answer = JSON.parse(JSON.stringify(this.$store.state.part9))
  ...
  beforeRouteLeave(to, from, next) {
    this.$store.commit('setTime', { id: 9, timeUsing: this.info.timeUsing })
    next()
  },
  methods: {
    onChange(val, num) {
      this.info.answer[0] = val
      this.$store.commit('addAnswer9', this.info.answer)
      if (num >= 1) {
        this.$emit('curr', true, 8)
      } else {
        this.$emit('curr', false, 8)
      }
    },
    // 获取题目
    async getWtire() {
